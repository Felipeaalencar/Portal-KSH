...
async function salvarSenha(token) {
  ...
  const d = await r.json();
  if (r.ok) { ME = {email:d.email,nome:d.email.split('@')[0],funcao:'Gestor',ini:d.email.substring(0,2).toUpperCase(),token}; sessionStorage.setItem('ksh_me',JSON.stringify(ME)); iniciarApp(); } function despGeralArquivoLido(input){const f=input.files&&input.files[0];if(!f)return;const reader=new FileReader();reader.onload=function(e){const res=e.target.result;const parts=res.split(',');despGeralState.mediaType=f.type||'image/jpeg';despGeralState.base64=parts[1];const prev=document.getElementById('desp-preview');if(prev){prev.src=res;prev.style.display='block';}const btnAuto=document.getElementById('desp-btn-auto');if(btnAuto)btnAuto.disabled=false;const btnCont=document.getElementById('desp-btn-cont-doc');if(btnCont)btnCont.disabled=false;};reader.readAsDataURL(f);} async function despGeralPreencherAuto(){if(!despGeralState.base64){toast('Anexe um documento primeiro','err');return;}const btn=document.getElementById('desp-btn-auto');if(btn){btn.disabled=true;btn.textContent='Lendo documento...';}try{const r=await fetch(SB_URL+'/functions/v1/ocr-despesa',{method:'POST',headers:{'apikey':SB_KEY,'Authorization':'Bearer '+ME.token,'Content-Type':'application/json'},body:JSON.stringify({image_base64:despGeralState.base64,media_type:despGeralState.mediaType})});const d=await r.json();if(d.empresa)document.getElementById('desp-inp-empresa').value=d.empresa;if(d.valor)document.getElementById('desp-inp-valor').value=d.valor;if(d.data)document.getElementById('desp-inp-data').value=d.data;if(d.descricao)document.getElementById('desp-inp-descricao').value=d.descricao;if(d.tipo_documento)document.getElementById('desp-inp-tipo').value=d.tipo_documento;toast('Documento lido com sucesso!','ok');despGeralIrPara(2);}catch(e){toast('Nao consegui ler o documento automaticamente. Preencha manualmente.','err');despGeralIrPara(2);}finally{if(btn){btn.disabled=false;btn.textContent='Preencher automaticamente';}}} function despGeralIrPara(step){despGeralState.step=step;['doc','comp','conf'].forEach((k,i)=>{const el=document.getElementById('desp-step-'+k);if(el)el.style.display=(i+1===step)?'block':'none';});document.querySelectorAll('.desp-tab').forEach((t,i)=>{t.classList.toggle('on',(i+1)===step);});if(step===3)despGeralMontarResumo();} function despGeralMontarResumo(){const el=document.getElementById('desp-resumo');if(!el)return;const empresa=document.getElementById('desp-inp-empresa').value||'-';const valor=document.getElementById('desp-inp-valor').value||'0';const data=document.getElementById('desp-inp-data').value||'-';const descricao=document.getElementById('desp-inp-descricao').value||'-';const tipo=document.getElementById('desp-inp-tipo').value||'Documento';el.innerHTML='<div style="display:flex;flex-direction:column;gap:8px;font-size:13px"><div><b>Tipo:</b> '+tipo+'</div><div><b>Empresa:</b> '+empresa+'</div><div><b>Descricao:</b> '+descricao+'</div><div><b>Data:</b> '+data+'</div><div><b>Valor:</b> $'+Number(valor).toLocaleString('en-US',{minimumFractionDigits:2})+'</div></div>';} async function despGeralSalvar(){const empresa=document.getElementById('desp-inp-empresa').value.trim();const valor=parseFloat(document.getElementById('desp-inp-valor').value)||0;const data=document.getElementById('desp-inp-data').value||null;const descricao=document.getElementById('desp-inp-descricao').value.trim();const tipo=document.getElementById('desp-inp-tipo').value;if(!empresa||!valor){toast('Preencha empresa e valor','err');return;}const btn=document.getElementById('desp-btn-salvar');if(btn){btn.disabled=true;btn.textContent='Salvando...';}try{let documento_url=null;if(despGeralState.base64){const fname=Date.now()+'_'+Math.random().toString(36).slice(2)+'.jpg';const bin=atob(despGeralState.base64);const bytes=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);const up=await fetch(SB_URL+'/storage/v1/object/despesas-docs/'+fname,{method:'POST',headers:{'apikey':SB_KEY,'Authorization':'Bearer '+ME.token,'Content-Type':despGeralState.mediaType||'image/jpeg'},body:bytes});if(up.ok)documento_url=SB_URL+'/storage/v1/object/public/despesas-docs/'+fname;}await sbPost('despesas_gerais',{empresa,valor,data_documento:data,descricao,tipo_documento:tipo,documento_url,lancado_por:ME?ME.nome:''});toast('Despesa lancada!','ok');despGeralState={step:1,base64:null,mediaType:null};renderDespesasGerais();}catch(e){toast('Erro: '+e.message,'err');}finally{if(btn){btn.disabled=false;btn.textContent='Salvar despesa';}}} async function carregarMinhasDespesasGerais(){const el=document.getElementById('desp-lista');if(!el)return;try{const rows=await sbGet('despesas_gerais?order=criado_em.desc&limit=50');el.innerHTML=rows.length?rows.map(d=>'<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;border-bottom:0.5px solid var(--border)"><div><div style="font-size:13px;font-weight:500">'+(d.tipo_documento||'Documento')+' - '+(d.empresa||'-')+'</div><div style="font-size:11px;color:var(--text-muted);margin-top:2px">'+(d.lancado_por||'-')+' - '+(d.data_documento||'')+'</div></div><div style="text-align:right"><div style="font-weight:600">$'+Number(d.valor||0).toLocaleString('en-US',{minimumFractionDigits:2})+'</div></div></div>').join(''):'<div style="text-align:center;padding:40px;color:var(--text-muted)">Nenhuma despesa lancada ainda</div>';}catch(e){el.innerHTML='<div style="text-align:center;padding:40px;color:#e74c3c">Erro ao carregar: '+e.message+'</div>';}} let despGeralState={step:1,base64:null,mediaType:null}; function renderDespesasGerais(){despGeralState={step:1,base64:null,mediaType:null};const el=document.getElementById('mod-content');if(!el)return;el.innerHTML='<div style="max-width:720px"><div style="display:flex;gap:8px;margin-bottom:16px;background:var(--surface-1);border:0.5px solid var(--border);border-radius:var(--radius);overflow:hidden"><div class="desp-tab on" onclick="despGeralIrPara(1)" style="flex:1;text-align:center;padding:10px;font-size:12px;cursor:pointer">Documento</div><div class="desp-tab" onclick="despGeralIrPara(2)" style="flex:1;text-align:center;padding:10px;font-size:12px;cursor:pointer">Complementar</div><div class="desp-tab" onclick="despGeralIrPara(3)" style="flex:1;text-align:center;padding:10px;font-size:12px;cursor:pointer">Confirmar</div></div><div id="desp-step-doc" style="background:var(--surface-2);border:0.5px solid var(--border);border-radius:12px;padding:16px;margin-bottom:16px"><div style="font-size:12px;font-weight:500;margin-bottom:10px">Foto do Documento Fiscal</div><div style="border:1.5px dashed var(--border);border-radius:10px;padding:24px;text-align:center;margin-bottom:12px"><img id="desp-preview" style="display:none;max-width:100%;max-height:200px;border-radius:8px;margin-bottom:10px"><div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap"><label style="padding:8px 14px;border:0.5px solid var(--border);border-radius:8px;cursor:pointer;font-size:12px">Camera<input type="file" accept="image/*,application/pdf" style="display:none" onchange="despGeralArquivoLido(this)"></label><label style="padding:8px 14px;border:0.5px solid var(--border);border-radius:8px;cursor:pointer;font-size:12px">Galeria/Arquivo<input type="file" accept="image/*" style="display:none" onchange="despGeralArquivoLido(this)"></label></div></div><button id="desp-btn-auto" class="btn-pri" disabled onclick="despGeralPreencherAuto()" style="width:100%;margin-bottom:10px">Preencher automaticamente</button><button id="desp-btn-cont-doc" class="btn-sec" disabled onclick="despGeralIrPara(2)" style="width:100%">Preencher manualmente</button></div><div id="desp-step-comp" style="display:none;background:var(--surface-2);border:0.5px solid var(--border);border-radius:12px;padding:16px;margin-bottom:16px"><div style="font-size:12px;font-weight:500;margin-bottom:12px">Complementar dados</div><div style="display:flex;flex-direction:column;gap:10px"><select id="desp-inp-tipo" style="padding:8px 10px;border:0.5px solid var(--border);border-radius:8px"><option value="Documento">Documento</option><option value="Cupom Fiscal (NFC-e)">Cupom Fiscal (NFC-e)</option><option value="Nota de Produto (NF-e)">Nota de Produto (NF-e)</option></select><input id="desp-inp-empresa" placeholder="Empresa / Fornecedor" style="padding:8px 10px;border:0.5px solid var(--border);border-radius:8px"><input id="desp-inp-descricao" placeholder="Descricao" style="padding:8px 10px;border:0.5px solid var(--border);border-radius:8px"><input id="desp-inp-valor" type="number" step="0.01" placeholder="Valor" style="padding:8px 10px;border:0.5px solid var(--border);border-radius:8px"><input id="desp-inp-data" type="date" style="padding:8px 10px;border:0.5px solid var(--border);border-radius:8px"></div><div style="display:flex;gap:10px;margin-top:14px"><button class="btn-sec" onclick="despGeralIrPara(1)" style="flex:1">Voltar</button><button class="btn-pri" onclick="despGeralIrPara(3)" style="flex:1">Continuar</button></div></div><div id="desp-step-conf" style="display:none;background:var(--surface-2);border:0.5px solid var(--border);border-radius:12px;padding:16px;margin-bottom:16px"><div style="font-size:12px;font-weight:500;margin-bottom:12px">Confirmar lancamento</div><div id="desp-resumo"></div><div style="display:flex;gap:10px;margin-top:14px"><button class="btn-sec" onclick="despGeralIrPara(2)" style="flex:1">Voltar</button><button id="desp-btn-salvar" class="btn-pri" onclick="despGeralSalvar()" style="flex:1">Salvar despesa</button></div></div><div style="background:var(--surface-2);border:0.5px solid var(--border);border-radius:12px;overflow:hidden"><div style="padding:12px 14px;border-bottom:0.5px solid var(--border);font-size:12px;font-weight:500">Despesas Lancadas</div><div id="desp-lista"></div></div></div>';carregarMinhasDespesasGerais();}
}
