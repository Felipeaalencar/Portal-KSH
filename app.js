...
let ME = null, googleToken = sessionStorage.getItem('ksh_drive_token') || null; let despesasFolderId = null; async function getPastaDespesas() { if (despesasFolderId) return despesasFolderId; const cached = sessionStorage.getItem('ksh_despesas_folder_id'); if (cached) { despesasFolderId = cached; return despesasFolderId; } try { const parentId = await getPastaPortal(); const q = encodeURIComponent("name='Despesas' and mimeType='application/vnd.google-apps.folder' and trashed=false and '" + parentId + "' in parents"); const r = await fetch('https://www.googleapis.com/drive/v3/files?q=' + q + '&fields=files(id,name)', { headers: { 'Authorization': 'Bearer ' + googleToken } }); const d = await r.json(); if (d.files && d.files.length) despesasFolderId = d.files[0].id; else despesasFolderId = await criarPastaDrive('Despesas', parentId); } catch(e) { const parentId = await getPastaPortal(); despesasFolderId = await criarPastaDrive('Despesas', parentId); } if (despesasFolderId) sessionStorage.setItem('ksh_despesas_folder_id', despesasFolderId); return despesasFolderId; } function despGeralArquivoLido(input){   const f = input.files && input.files[0];   if (!f) return;   despGeralState.file = f;   const reader = new FileReader();   reader.onload = function(e){     const res = e.target.result;     const parts = res.split(',');     despGeralState.mediaType = f.type || 'image/jpeg';     despGeralState.base64 = parts[1];     const isPdf = despGeralState.mediaType === 'application/pdf';     const prev = document.getElementById('desp-preview');     const prevDoc = document.getElementById('desp-preview-doc');     if (isPdf) { if (prev) prev.style.display = 'none'; if (prevDoc) { prevDoc.style.display = 'flex'; const fn = document.getElementById('desp-preview-filename'); if (fn) fn.textContent = f.name; } } else { if (prevDoc) prevDoc.style.display = 'none'; if (prev) { prev.src = res; prev.style.display = 'block'; } }     const btnAuto = document.getElementById('desp-btn-auto');     if (btnAuto) btnAuto.disabled = false;     const btnCont = document.getElementById('desp-btn-cont-doc');     if (btnCont) btnCont.disabled = false;   };   reader.readAsDataURL(f); }  async function despGeralPreencherAuto(){   if (!despGeralState.base64) { toast('Anexe um documento primeiro','err'); return; }   if (despGeralState.mediaType === 'application/pdf') { toast('Leitura automatica funciona so com fotos por enquanto. Preencha os campos manualmente.','err'); return; }   const btn = document.getElementById('desp-btn-auto');   if (btn) { btn.textContent = 'Lendo documento...'; btn.disabled = true; }   try {     const r = await fetch(SB_URL + '/functions/v1/ocr-despesa', {       method: 'POST',       headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + ME.token, 'Content-Type': 'application/json' },       body: JSON.stringify({ image_base64: despGeralState.base64, media_type: despGeralState.mediaType })     });     const d = await r.json();     if (d.empresa) document.getElementById('desp-empresa').value = d.empresa;     if (d.valor) document.getElementById('desp-valor').value = d.valor;     if (d.data) document.getElementById('desp-data').value = d.data;     if (d.descricao) document.getElementById('desp-descricao').value = d.descricao;     if (d.tipo_documento) document.getElementById('desp-tipo').value = d.tipo_documento;     toast('Preenchido automaticamente!','ok');   } catch(e) { toast('Erro ao ler documento: ' + e.message, 'err'); }   if (btn) { btn.textContent = 'Preencher automaticamente'; btn.disabled = false; } }  function despGeralIrPara(step){   despGeralState.step = step;   document.querySelectorAll('.desp-step').forEach(function(el){ el.style.display = 'none'; });   const target = document.getElementById('desp-step-' + step);   if (target) target.style.display = 'block';   document.querySelectorAll('.desp-step-dot').forEach(function(el){ el.classList.remove('on'); });   document.querySelectorAll('.desp-step-dot').forEach(function(el){     if (parseInt(el.dataset.step) <= step) el.classList.add('on');   });   if (step === 3) despGeralMontarResumo(); }  function despGeralMontarResumo(){   const empresa = (document.getElementById('desp-empresa')?.value || '').trim() || '—';   const valor = document.getElementById('desp-valor')?.value || '0';   const data = document.getElementById('desp-data')?.value || '—';   const descricao = (document.getElementById('desp-descricao')?.value || '').trim() || '—';   const tipo = document.getElementById('desp-tipo')?.value || '—';   const el = document.getElementById('desp-resumo');   if (!el) return;   el.innerHTML = '<div style="display:flex;flex-direction:column;gap:10px">' +     '<div><div style="font-size:9px;color:#888;text-transform:uppercase;letter-spacing:.5px">Empresa</div><div style="font-size:14px;font-weight:600">' + empresa + '</div></div>' +     '<div><div style="font-size:9px;color:#888;text-transform:uppercase;letter-spacing:.5px">Valor</div><div style="font-size:14px;font-weight:600">R$ ' + Number(valor).toFixed(2) + '</div></div>' +     '<div><div style="font-size:9px;color:#888;text-transform:uppercase;letter-spacing:.5px">Data</div><div style="font-size:13px">' + data + '</div></div>' +     '<div><div style="font-size:9px;color:#888;text-transform:uppercase;letter-spacing:.5px">Tipo</div><div style="font-size:13px">' + tipo + '</div></div>' +     '<div><div style="font-size:9px;color:#888;text-transform:uppercase;letter-spacing:.5px">Descricao</div><div style="font-size:13px;white-space:pre-wrap">' + descricao + '</div></div>' +   '</div>'; }  async function despGeralSalvar(){   const btn = document.getElementById('desp-btn-salvar');   if (btn) { btn.textContent = 'Salvando...'; btn.disabled = true; }   try {     let documento_url = null;     if (despGeralState.file) {       try {         const folderId = await getPastaDespesas();         const dr = await uploadDrive(despGeralState.file, folderId);         if (dr && dr.id) documento_url = 'https://drive.google.com/file/d/' + dr.id + '/view';       } catch(e) {}     }     await sbPost('despesas_gerais', {       documento_url: documento_url,       tipo_documento: document.getElementById('desp-tipo')?.value || null,       empresa: (document.getElementById('desp-empresa')?.value || '').trim() || null,       descricao: (document.getElementById('desp-descricao')?.value || '').trim() || null,       valor: Number(document.getElementById('desp-valor')?.value || 0),       data_documento: document.getElementById('desp-data')?.value || null,       lancado_por: ME.nome     });     toast('Despesa lancada!', 'ok');     despGeralState = { step: 1, base64: null, mediaType: null, file: null };     renderDespesasGerais();   } catch(e) { toast('Erro: ' + e.message, 'err'); if (btn) { btn.textContent = 'Salvar despesa'; btn.disabled = false; } } }  async function excluirDespesaGeral(id){ if (ME.funcao !== 'Gestor') { toast('Apenas gestores podem excluir despesas', 'err'); return; } if (!confirm('Excluir esta despesa lancada?')) return; try { await sbDelete('despesas_gerais?id=eq.' + id); toast('Despesa excluida', 'ok'); carregarMinhasDespesasGerais(); } catch(e) { toast('Erro: ' + e.message, 'err'); } }  async function carregarMinhasDespesasGerais(){   const el = document.getElementById('desp-lista');   if (!el) return;   el.innerHTML = '<div style="text-align:center;padding:20px;color:#bbb;font-size:12px">Carregando...</div>';   try {     const lista = await sbGet('despesas_gerais?order=criado_em.desc&limit=50');     if (!lista.length) { el.innerHTML = '<div style="text-align:center;padding:20px;color:#bbb;font-size:12px">Nenhuma despesa lancada ainda</div>'; return; }     el.innerHTML = lista.map(function(d){       return '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid #f5f5f3">' +         '<div><div style="font-size:13px;font-weight:600">' + (d.empresa||'Sem nome') + '</div>' + '<div style="font-size:11px;color:#888">' + (d.descricao||'').split(String.fromCharCode(10)).join(' | ') + ' · ' + (d.data_documento||'') + ' · ' + (d.lancado_por||'') + (d.documento_url ? ' · <a href="'+d.documento_url+'" target="_blank" style="color:#2563eb;text-decoration:none">Ver documento</a>' : '') + '</div></div>' +         '<div style="font-size:14px;font-weight:600">R$ ' + Number(d.valor||0).toFixed(2) + '</div>' + (ME.funcao==='Gestor' ? '<button onclick="excluirDespesaGeral(\''+d.id+'\')" style="background:none;border:none;cursor:pointer;color:#e74c3c;font-size:16px;padding:4px 6px;margin-left:6px" title="Excluir">🗑️</button>' : '') +       '</div>';     }).join('');   } catch(e) { el.innerHTML = '<div style="text-align:center;padding:20px;color:#e74c3c;font-size:12px">Erro: ' + e.message + '</div>'; } }  let despGeralState = { step: 1, base64: null, mediaType: null, file: null };  let frotaFolderId = null; async function getPastaFrota() { if (frotaFolderId) return frotaFolderId; const cached = sessionStorage.getItem('ksh_frota_folder_id'); if (cached) { frotaFolderId = cached; return frotaFolderId; } try { const parentId = await getPastaPortal(); const q = encodeURIComponent("name='Frota' and mimeType='application/vnd.google-apps.folder' and trashed=false and '" + parentId + "' in parents"); const r = await fetch('https://www.googleapis.com/drive/v3/files?q=' + q + '&fields=files(id,name)', { headers: { 'Authorization': 'Bearer ' + googleToken } }); const d = await r.json(); if (d.files && d.files.length) frotaFolderId = d.files[0].id; else frotaFolderId = await criarPastaDrive('Frota', parentId); } catch(e) { const parentId = await getPastaPortal(); frotaFolderId = await criarPastaDrive('Frota', parentId); } if (frotaFolderId) sessionStorage.setItem('ksh_frota_folder_id', frotaFolderId); return frotaFolderId; } let frotaVeiculosCache = []; let frotaState = { veiculoAtual: null, base64: null, mediaType: null, file: null }; const FROTA_CATEGORIAS = ['Combustível','Manutenção','Seguro','Pneus','Licenciamento','Outros']; const FROTA_CORES = { 'Combustível':'#2a78d6', 'Manutenção':'#eb6834', 'Seguro':'#1baf7a', 'Pneus':'#eda100', 'Licenciamento':'#e87ba4', 'Outros':'#008300' }; function calcularMPGs(lista){ const ordenada = lista.slice().sort(function(a,b){ return new Date(a.data_documento) - new Date(b.data_documento); }); const mpgs = []; for (let i=1;i<ordenada.length;i++){ const antes = Number(ordenada[i-1].milhas_informada); const depois = Number(ordenada[i].milhas_informada); const galoes = Number(ordenada[i].galoes); if (depois > antes && galoes > 0){ mpgs.push({ data: ordenada[i].data_documento, mpg: (depois - antes) / galoes }); } } return mpgs; } async function renderFrota(){ const el = document.getElementById('mod-content'); el.innerHTML = '<div style="text-align:center;padding:20px;color:#bbb;font-size:12px">Carregando frota...</div>'; try { const veiculos = await sbGet('veiculos?order=criado_em.desc'); frotaVeiculosCache = veiculos; const ativos = veiculos.filter(function(v){ return v.status === 'Ativo'; }).length; el.innerHTML = '<div style="display:flex;justify-content:flex-end;margin-bottom:14px">' + (ME.funcao === 'Gestor' ? '<button class="btn-pri" onclick="frotaAbrirNovo()">+ Novo veiculo</button>' : '') + '</div>' + '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:20px">' + '<div style="background:#f7f7f5;border-radius:8px;padding:14px"><div style="font-size:11px;color:#888">Veiculos ativos</div><div style="font-size:20px;font-weight:600">' + ativos + ' <span style="font-size:12px;color:#aaa;font-weight:400">de ' + veiculos.length + '</span></div></div>' + '<div id="frota-kpi-milhas" style="background:#f7f7f5;border-radius:8px;padding:14px"><div style="font-size:11px;color:#888">Milhas rodadas (mes)</div><div style="font-size:20px;font-weight:600">-</div></div>' + '<div id="frota-kpi-gasto" style="background:#f7f7f5;border-radius:8px;padding:14px"><div style="font-size:11px;color:#888">Gasto total (mes)</div><div style="font-size:20px;font-weight:600">-</div></div>' + '<div id="frota-kpi-mpg" style="background:#f7f7f5;border-radius:8px;padding:14px"><div style="font-size:11px;color:#888">Consumo medio</div><div style="font-size:20px;font-weight:600">-</div></div>' + '</div>' + '<div id="frota-lista" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px"></div>'; document.getElementById('frota-lista').innerHTML = veiculos.length ? veiculos.map(function(v){ return '<div onclick="frotaAbrirVeiculo(\'' + v.id + '\')" style="cursor:pointer;background:#fff;border:1px solid #e8e8e5;border-radius:10px;padding:14px">' + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">' + '<div style="font-weight:600;font-size:14px">' + (v.marca||'') + ' ' + (v.modelo||'') + ' ' + (v.ano||'') + '</div>' + '<span style="font-size:11px;padding:2px 8px;border-radius:6px;background:' + (v.status==='Ativo'?'#e6f6ec':'#fdf2e3') + ';color:' + (v.status==='Ativo'?'#1a7f4e':'#946200') + '">' + (v.status||'Ativo') + '</span>' + '</div>' + '<div style="font-size:12px;color:#888">Placa ' + (v.placa||'-') + '</div>' + '<div style="font-size:12px;color:#888">' + (v.responsavel||'Sem responsavel') + ' - ' + Number(v.milhas_atual||0).toLocaleString() + ' mi</div>' + (ME.funcao==='Gestor' ? '<div style="text-align:right;margin-top:8px"><button onclick="event.stopPropagation(); excluirVeiculo(\'' + v.id + '\')" style="background:none;border:none;cursor:pointer;color:#e74c3c;font-size:12px">Excluir veiculo</button></div>' : '') + '</div>'; }).join('') : '<div style="text-align:center;padding:30px;color:#bbb;font-size:12px">Nenhum veiculo cadastrado ainda</div>'; frotaAtualizarKpis(); } catch(e) { el.innerHTML = '<div style="text-align:center;padding:20px;color:#e74c3c;font-size:12px">Erro: ' + e.message + '</div>'; } } async function frotaAtualizarKpis(){ try { const desde = new Date(); desde.setDate(1); const desdeStr = desde.toISOString().slice(0,10); const despesasMes = await sbGet('despesas_veiculos?data_documento=gte.' + desdeStr); const gasto = despesasMes.reduce(function(s,d){ return s + Number(d.valor||0); }, 0); const elGasto = document.getElementById('frota-kpi-gasto'); if (elGasto) elGasto.querySelector('div:last-child').textContent = '$' + gasto.toLocaleString(undefined,{maximumFractionDigits:0}); const porVeiculoMes = {}; despesasMes.forEach(function(d){ if (d.milhas_informada) (porVeiculoMes[d.veiculo_id] = porVeiculoMes[d.veiculo_id] || []).push(Number(d.milhas_informada)); }); let milhasRodadas = 0; Object.keys(porVeiculoMes).forEach(function(vid){ const arr = porVeiculoMes[vid]; if (arr.length>1) milhasRodadas += Math.max.apply(null,arr) - Math.min.apply(null,arr); }); const elMilhas = document.getElementById('frota-kpi-milhas'); if (elMilhas) elMilhas.querySelector('div:last-child').textContent = milhasRodadas.toLocaleString(); const todasCombustivel = await sbGet('despesas_veiculos?categoria=eq.' + encodeURIComponent('Combustível') + '&order=data_documento.asc'); const porVeiculo = {}; todasCombustivel.forEach(function(d){ (porVeiculo[d.veiculo_id] = porVeiculo[d.veiculo_id] || []).push(d); }); const medias = Object.keys(porVeiculo).map(function(vid){ const mpgs = calcularMPGs(porVeiculo[vid]); return mpgs.length ? mpgs[mpgs.length-1].mpg : null; }).filter(function(v){ return v; }); const elMpg = document.getElementById('frota-kpi-mpg'); if (elMpg) elMpg.querySelector('div:last-child').innerHTML = medias.length ? (medias.reduce(function(a,b){return a+b;},0)/medias.length).toFixed(1) + ' <span style="font-size:12px;color:#aaa;font-weight:400">mpg</span>' : '-'; } catch(e) {} } function frotaAbrirNovo(){ const el = document.getElementById('mod-content'); function campo(id,label,type){ return '<div><label style="font-size:11px;color:#888;display:block;margin-bottom:2px">' + label + '</label><input id="fn-' + id + '" type="' + (type||'text') + '" style="width:100%;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px"></div>'; } el.innerHTML = '<button onclick="renderFrota()" style="background:none;border:none;color:#888;font-size:12px;cursor:pointer;margin-bottom:14px">← Voltar para frota</button>' + '<div style="background:#fff;border:1px solid #e8e8e5;border-radius:10px;padding:16px 18px;max-width:560px">' + '<div style="font-weight:600;font-size:14px;margin-bottom:12px">Novo veiculo</div>' + '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px">' + campo('placa','Placa') + campo('estado','Estado') + campo('marca','Marca') + campo('modelo','Modelo') + campo('ano','Ano','number') + campo('cor','Cor') + campo('vin','VIN') + campo('milhas_atual','Milhas atuais','number') + campo('data_aquisicao','Aquisicao','date') + campo('responsavel','Responsavel') + campo('seguro_vencimento','Seguro ate','date') + '</div>' + '<div style="margin-top:14px"><button class="btn-pri" id="fn-btn-salvar" onclick="frotaSalvarNovo()">Salvar veiculo</button></div>' + '</div>'; } async function frotaSalvarNovo(){ const btnNovo = document.getElementById('fn-btn-salvar'); if (btnNovo) { if (btnNovo.dataset.saving === '1') return; btnNovo.dataset.saving = '1'; btnNovo.textContent = 'Salvando...'; } try { const body = { placa: document.getElementById('fn-placa').value.trim(), estado: document.getElementById('fn-estado').value.trim() || null, marca: document.getElementById('fn-marca').value.trim() || null, modelo: document.getElementById('fn-modelo').value.trim() || null, ano: Number(document.getElementById('fn-ano').value || 0) || null, cor: document.getElementById('fn-cor').value.trim() || null, vin: document.getElementById('fn-vin').value.trim() || null, milhas_atual: Number(document.getElementById('fn-milhas_atual').value || 0), data_aquisicao: document.getElementById('fn-data_aquisicao').value || null, responsavel: document.getElementById('fn-responsavel').value.trim() || null, seguro_vencimento: document.getElementById('fn-seguro_vencimento').value || null, status: 'Ativo' }; if (!body.placa) { toast('Informe a placa', 'err'); if (btnNovo) { btnNovo.dataset.saving = ''; btnNovo.textContent = 'Salvar veiculo'; } return; } await sbPost('veiculos', body); toast('Veiculo cadastrado', 'ok'); renderFrota(); } catch(e) { toast('Erro: ' + e.message, 'err'); if (btnNovo) { btnNovo.dataset.saving = ''; btnNovo.textContent = 'Salvar veiculo'; } } } async function frotaAbrirVeiculo(id){ const v = frotaVeiculosCache.find(function(x){ return x.id === id; }); if (!v) return; frotaState.veiculoAtual = v; frotaState.base64 = null; frotaState.mediaType = null; frotaState.file = null; const el = document.getElementById('mod-content'); el.innerHTML = '<div style="text-align:center;padding:20px;color:#bbb;font-size:12px">Carregando...</div>'; let despesas = []; try { despesas = await sbGet('despesas_veiculos?veiculo_id=eq.' + id + '&order=data_documento.desc'); } catch(e) {} const combustivel = despesas.filter(function(d){ return d.categoria === 'Combustível'; }); const mpgHist = calcularMPGs(combustivel); const mpgAtual = mpgHist.length ? mpgHist[mpgHist.length-1].mpg : null; el.innerHTML = '<button onclick="renderFrota()" style="background:none;border:none;color:#888;font-size:12px;cursor:pointer;margin-bottom:14px">← Voltar para frota</button>' + '<div id="frota-cadastro" style="background:#fff;border:1px solid #e8e8e5;border-radius:10px;padding:16px 18px;margin-bottom:18px"></div>' + '<div style="display:grid;grid-template-columns:1.2fr 1fr;gap:16px;margin-bottom:20px">' + '<div style="background:#fff;border:1px solid #e8e8e5;border-radius:10px;padding:16px 18px"><div style="font-weight:600;font-size:13px;margin-bottom:12px">Lançar despesa por foto</div><div id="frota-desp-form"></div></div>' + '<div style="background:#f7f7f5;border-radius:10px;padding:16px 18px"><div style="font-weight:600;font-size:13px;margin-bottom:8px">Consumo medio</div><div style="font-size:20px;font-weight:600">' + (mpgAtual ? mpgAtual.toFixed(1) + ' mpg' : '-') + '</div><div style="font-size:11px;color:#888;margin-top:6px">Calculado a cada abastecimento</div></div>' + '</div>' + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px">' + '<div><div style="font-weight:600;font-size:13px;margin-bottom:8px">Gastos por categoria</div><canvas id="frota-chart-cat" height="180"></canvas></div>' + '<div><div style="font-weight:600;font-size:13px;margin-bottom:8px">Principais fornecedores</div><canvas id="frota-chart-forn" height="180"></canvas></div>' + '</div>' + '<div style="margin-bottom:20px"><div style="font-weight:600;font-size:13px;margin-bottom:8px">Consumo (mpg) por abastecimento</div><canvas id="frota-chart-mpg" height="140"></canvas></div>' + '<div style="font-weight:600;font-size:13px;margin-bottom:8px">Despesas lançadas</div>' + '<div id="frota-desp-lista"></div>'; frotaRenderCadastro(v); frotaRenderFormDespesa(); frotaRenderListaDespesas(despesas); frotaRenderCharts(despesas, mpgHist); } function frotaRenderCadastro(v){ const el = document.getElementById('frota-cadastro'); if (!el) return; function campo(label,val){ return '<div><div style="color:#888;margin-bottom:2px">' + label + '</div><div style="font-weight:500">' + (val || '-') + '</div></div>'; } const editavel = ME.funcao === 'Gestor'; el.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">' + '<div style="font-weight:600;font-size:15px">' + (v.marca||'') + ' ' + (v.modelo||'') + ' ' + (v.ano||'') + ' · ' + (v.placa||'') + '</div>' + (editavel ? '<button onclick="frotaToggleEdicao()" style="background:none;border:1px solid #ddd;border-radius:6px;padding:4px 10px;font-size:11px;cursor:pointer">Editar</button>' : '') + '</div>' + '<div id="frota-cadastro-view" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;font-size:12px">' + campo('Estado', v.estado) + campo('Cor', v.cor) + campo('VIN', v.vin) + campo('Milhas atuais', Number(v.milhas_atual||0).toLocaleString()) + campo('Aquisicao', v.data_aquisicao) + campo('Responsavel', v.responsavel) + campo('Seguro ate', v.seguro_vencimento) + campo('Status', v.status) + '</div>' + '<div id="frota-cadastro-edit" style="display:none"></div>'; } function frotaToggleEdicao(){ const v = frotaState.veiculoAtual; const view = document.getElementById('frota-cadastro-view'); const edit = document.getElementById('frota-cadastro-edit'); if (!view || !edit) return; function campo(id,label,val,type){ return '<div><label style="font-size:11px;color:#888;display:block;margin-bottom:2px">' + label + '</label><input id="fe-' + id + '" type="' + (type||'text') + '" value="' + (val==null?'':val) + '" style="width:100%;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px"></div>'; } function selectStatus(val){ return '<div><label style="font-size:11px;color:#888;display:block;margin-bottom:2px">Status</label><select id="fe-status" style="width:100%;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px"><option' + (val==='Ativo'?' selected':'') + '>Ativo</option><option' + (val==='Manutenção'?' selected':'') + '>Manutenção</option><option' + (val==='Inativo'?' selected':'') + '>Inativo</option></select></div>'; } if (edit.style.display === 'none') { view.style.display = 'none'; edit.style.display = 'grid'; edit.style.gridTemplateColumns = 'repeat(auto-fit,minmax(140px,1fr))'; edit.style.gap = '10px'; edit.innerHTML = campo('estado','Estado', v.estado) + campo('cor','Cor', v.cor) + campo('vin','VIN', v.vin) + campo('milhas_atual','Milhas atuais', v.milhas_atual) + campo('data_aquisicao','Aquisicao', v.data_aquisicao,'date') + campo('responsavel','Responsavel', v.responsavel) + campo('seguro_vencimento','Seguro ate', v.seguro_vencimento,'date') + selectStatus(v.status) + '<div style="grid-column:1/-1;display:flex;gap:8px;margin-top:6px"><button class="btn-pri" onclick="frotaSalvarEdicao()">Salvar</button><button onclick="frotaToggleEdicao()" style="background:none;border:1px solid #ddd;border-radius:6px;padding:6px 14px;font-size:12px;cursor:pointer">Cancelar</button></div>'; } else { edit.style.display = 'none'; view.style.display = 'grid'; } } async function frotaSalvarEdicao(){ const v = frotaState.veiculoAtual; try { const body = { estado: document.getElementById('fe-estado').value || null, cor: document.getElementById('fe-cor').value || null, vin: document.getElementById('fe-vin').value || null, milhas_atual: Number(document.getElementById('fe-milhas_atual').value || 0), data_aquisicao: document.getElementById('fe-data_aquisicao').value || null, responsavel: document.getElementById('fe-responsavel').value || null, seguro_vencimento: document.getElementById('fe-seguro_vencimento').value || null, status: document.getElementById('fe-status').value }; await sbPatch('veiculos?id=eq.' + v.id, body); toast('Veiculo atualizado', 'ok'); Object.assign(v, body); frotaRenderCadastro(v); } catch(e) { toast('Erro: ' + e.message, 'err'); } } function frotaRenderFormDespesa(){ const el = document.getElementById('frota-desp-form'); if (!el) return; function campo(id,type){ if (id === 'categoria') return '<select id="frota-categoria" style="width:100%;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px"></select>'; const labels = { fornecedor:'Fornecedor', valor:'Valor ($)', data:'Data', milhas:'Milhas no odometro', galoes:'Galoes' }; return '<input id="frota-' + id + '" type="' + (type||'text') + '" placeholder="' + labels[id] + '" style="width:100%;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px">'; } el.innerHTML = '<input type="file" accept="image/*" id="frota-file" onchange="frotaArquivoLido(this)" style="font-size:12px;margin-bottom:10px">' + '<div id="frota-preview" style="display:none;margin-bottom:10px"><img id="frota-preview-img" style="max-width:120px;border-radius:6px;border:1px solid #eee"></div>' + '<div><button id="frota-btn-auto" onclick="frotaPreencherAuto()" disabled style="background:none;border:1px solid #ddd;border-radius:6px;padding:6px 12px;font-size:12px;cursor:pointer;margin-bottom:12px">Preencher automaticamente</button></div>' + '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px">' + campo('categoria') + campo('fornecedor') + campo('valor','number') + campo('data','date') + campo('milhas','number') + campo('galoes','number') + '</div>' + '<textarea id="frota-descricao" placeholder="Descricao" style="width:100%;margin-top:8px;padding:6px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px;min-height:50px"></textarea>' + '<div style="margin-top:10px"><button class="btn-pri" id="frota-btn-salvar" onclick="frotaSalvarDespesa()">Salvar despesa</button></div>'; const sel = document.getElementById('frota-categoria'); if (sel) sel.innerHTML = '<option value="">Categoria</option>' + FROTA_CATEGORIAS.map(function(c){ return '<option value="' + c + '">' + c + '</option>'; }).join(''); } function frotaArquivoLido(input){ const f = input.files && input.files[0]; if (!f) return; frotaState.file = f; const reader = new FileReader(); reader.onload = function(e){ const result = e.target.result; frotaState.base64 = result.split(',')[1]; frotaState.mediaType = f.type; const img = document.getElementById('frota-preview-img'); const prev = document.getElementById('frota-preview'); if (img && prev) { img.src = result; prev.style.display = 'block'; } const btn = document.getElementById('frota-btn-auto'); if (btn) btn.disabled = false; }; reader.readAsDataURL(f); } async function frotaPreencherAuto(){ if (!frotaState.base64) { toast('Anexe uma foto primeiro', 'err'); return; } const btn = document.getElementById('frota-btn-auto'); if (btn) { btn.textContent = 'Lendo documento...'; btn.disabled = true; } try { const r = await fetch(SB_URL + '/functions/v1/ocr-despesa', { method: 'POST', headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + ME.token, 'Content-Type': 'application/json' }, body: JSON.stringify({ image_base64: frotaState.base64, media_type: frotaState.mediaType }) }); const d = await r.json(); if (d.empresa) document.getElementById('frota-fornecedor').value = d.empresa; if (d.valor) document.getElementById('frota-valor').value = d.valor; if (d.data) document.getElementById('frota-data').value = d.data; if (d.descricao) document.getElementById('frota-descricao').value = d.descricao; if (d.categoria && FROTA_CATEGORIAS.indexOf(d.categoria) !== -1) document.getElementById('frota-categoria').value = d.categoria; if (d.galoes) document.getElementById('frota-galoes').value = d.galoes; toast('Documento lido', 'ok'); } catch(e) { toast('Erro ao ler documento: ' + e.message, 'err'); } if (btn) { btn.textContent = 'Preencher automaticamente'; btn.disabled = false; } } async function frotaSalvarDespesa(){ const v = frotaState.veiculoAtual; const btn = document.getElementById('frota-btn-salvar'); if (btn) { btn.textContent = 'Salvando...'; btn.disabled = true; } try { let documento_url = null; if (frotaState.file) { try { const folderId = await getPastaFrota(); const dr = await uploadDrive(frotaState.file, folderId); if (dr && dr.id) documento_url = 'https://drive.google.com/file/d/' + dr.id + '/view'; } catch(e) {} } const milhas = Number(document.getElementById('frota-milhas').value || 0) || null; await sbPost('despesas_veiculos', { veiculo_id: v.id, categoria: document.getElementById('frota-categoria').value || null, fornecedor: document.getElementById('frota-fornecedor').value.trim() || null, valor: Number(document.getElementById('frota-valor').value || 0), data_documento: document.getElementById('frota-data').value || null, descricao: document.getElementById('frota-descricao').value.trim() || null, milhas_informada: milhas, galoes: Number(document.getElementById('frota-galoes').value || 0) || null, documento_url: documento_url, lancado_por: ME.nome }); if (milhas && milhas > Number(v.milhas_atual||0)) { await sbPatch('veiculos?id=eq.' + v.id, { milhas_atual: milhas }); v.milhas_atual = milhas; } toast('Despesa lancada', 'ok'); frotaAbrirVeiculo(v.id); } catch(e) { toast('Erro: ' + e.message, 'err'); if (btn) { btn.textContent = 'Salvar despesa'; btn.disabled = false; } } } function frotaRenderListaDespesas(despesas){ const el = document.getElementById('frota-desp-lista'); if (!el) return; if (!despesas.length) { el.innerHTML = '<div style="text-align:center;padding:20px;color:#bbb;font-size:12px">Nenhuma despesa lancada ainda</div>'; return; } el.innerHTML = despesas.map(function(d){ return '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f0f0ee;font-size:12px">' + '<div><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:' + (FROTA_CORES[d.categoria]||'#ccc') + ';margin-right:6px"></span>' + (d.fornecedor||'-') + ' <span style="color:#aaa">· ' + (d.categoria||'-') + '</span></div>' + '<div style="display:flex;align-items:center;gap:10px">' + '<span>$' + Number(d.valor||0).toFixed(2) + '</span>' + (d.documento_url ? '<a href="' + d.documento_url + '" target="_blank" style="color:#2a78d6;font-size:11px">Ver documento</a>' : '') + (ME.funcao==='Gestor' ? '<button onclick="excluirDespesaVeiculo(\'' + d.id + '\')" style="background:none;border:none;cursor:pointer;color:#e74c3c;font-size:14px" title="Excluir">🗑</button>' : '') + '</div>' + '</div>'; }).join(''); } async function excluirDespesaVeiculo(id){ if (ME.funcao !== 'Gestor') { toast('Apenas gestores podem excluir despesas', 'err'); return; } if (!confirm('Excluir esta despesa?')) return; try { await sbDelete('despesas_veiculos?id=eq.' + id); toast('Despesa excluida', 'ok'); frotaAbrirVeiculo(frotaState.veiculoAtual.id); } catch(e) { toast('Erro: ' + e.message, 'err'); } } let frotaCharts = {}; function frotaRenderCharts(despesas, mpgHist){ if (!window.Chart) return; Object.keys(frotaCharts).forEach(function(k){ if (frotaCharts[k]) frotaCharts[k].destroy(); }); const porCategoria = {}; despesas.forEach(function(d){ const c = d.categoria || 'Outros'; porCategoria[c] = (porCategoria[c]||0) + Number(d.valor||0); }); const catLabels = Object.keys(porCategoria); const elCat = document.getElementById('frota-chart-cat'); if (elCat && catLabels.length) { frotaCharts.cat = new Chart(elCat, { type: 'doughnut', data: { labels: catLabels, datasets: [{ data: catLabels.map(function(c){ return porCategoria[c]; }), backgroundColor: catLabels.map(function(c){ return FROTA_CORES[c] || '#ccc'; }) }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { font: { size: 10 } } } } } }); } const porFornecedor = {}; despesas.forEach(function(d){ const f = d.fornecedor || 'Outros'; porFornecedor[f] = (porFornecedor[f]||0) + Number(d.valor||0); }); const fornEntries = Object.entries(porFornecedor).sort(function(a,b){ return b[1]-a[1]; }).slice(0,5); const elForn = document.getElementById('frota-chart-forn'); if (elForn && fornEntries.length) { frotaCharts.forn = new Chart(elForn, { type: 'bar', data: { labels: fornEntries.map(function(e){ return e[0]; }), datasets: [{ data: fornEntries.map(function(e){ return e[1]; }), backgroundColor: '#2a78d6', borderRadius: 4 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } }); } const elMpg = document.getElementById('frota-chart-mpg'); if (elMpg && mpgHist.length) { frotaCharts.mpg = new Chart(elMpg, { type: 'line', data: { labels: mpgHist.map(function(m){ return m.data; }), datasets: [{ data: mpgHist.map(function(m){ return Number(m.mpg.toFixed(1)); }), borderColor: '#2a78d6', backgroundColor: 'rgba(42,120,214,0.1)', fill: true, tension: 0.3 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } }); } } async function excluirVeiculo(id){ if (ME.funcao !== 'Gestor') { toast('Apenas gestores podem excluir veiculos', 'err'); return; } if (!confirm('Excluir este veiculo e todas as despesas lancadas nele?')) return; try { await sbDelete('veiculos?id=eq.' + id); toast('Veiculo excluido', 'ok'); renderFrota(); } catch(e) { toast('Erro: ' + e.message, 'err'); } } function abrirNovaTarefaDeOS(osId){ const os = osData.find(function(o){ return o.id === osId; }); const clienteNome = (os && (os.cliente_nome || os.cliente)) || ''; const modal = document.createElement('div'); modal.id = 'modal-nova-tarefa-os'; modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:9999;display:flex;align-items:center;justify-content:center'; modal.innerHTML = '<div style="background:#fff;border-radius:12px;padding:20px;width:360px;max-width:90vw;font-family:inherit">' + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><div style="font-size:15px;font-weight:600">Nova tarefa</div><button onclick="document.getElementById(\'modal-nova-tarefa-os\').remove()" style="background:none;border:none;cursor:pointer;font-size:20px;color:#bbb">×</button></div>' + '<div style="font-size:12px;color:#888;margin-bottom:16px">' + ((os && os.titulo) || '') + (clienteNome ? ' · ' + clienteNome : '') + '</div>' + '<div style="background:#f9f9f7;border-radius:8px;padding:12px">' + '<div style="font-size:11px;font-weight:500;color:#444;margin-bottom:4px">O que precisa ser feito</div>' + '<input id="ntos-desc-input" placeholder="Ex: ligar para o cliente, cobrar restante..." style="width:100%;padding:7px 10px;border:1.5px solid #e8e8e5;border-radius:6px;font-size:12px;font-family:inherit;outline:none;margin-bottom:8px">' + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">' + '<div><div style="font-size:11px;font-weight:500;color:#444;margin-bottom:4px">Responsavel</div><input id="ntos-resp-input" value="' + ((ME && ME.nome) || '') + '" style="width:100%;padding:7px 10px;border:1.5px solid #e8e8e5;border-radius:6px;font-size:12px;font-family:inherit;outline:none"></div>' + '<div><div style="font-size:11px;font-weight:500;color:#444;margin-bottom:4px">Prazo (opcional)</div><input id="ntos-prazo-input" type="date" style="width:100%;padding:7px 10px;border:1.5px solid #e8e8e5;border-radius:6px;font-size:12px;font-family:inherit;outline:none"></div>' + '</div>' + '</div>' + '<div style="display:flex;gap:8px;justify-content:flex-end;margin-top:14px">' + '<button onclick="document.getElementById(\'modal-nova-tarefa-os\').remove()" style="padding:8px 16px;border:1px solid #e8e8e5;border-radius:7px;background:#fff;font-size:12px;cursor:pointer;font-family:inherit">Cancelar</button>' + '<button onclick="salvarNovaTarefaDeOS(\'' + osId + '\')" style="padding:8px 16px;border:none;border-radius:7px;background:#1a1a1a;color:#fff;font-size:12px;font-weight:500;cursor:pointer;font-family:inherit">Criar tarefa</button>' + '</div>' + '</div>'; document.body.appendChild(modal); setTimeout(function(){ const el = document.getElementById('ntos-desc-input'); if (el) el.focus(); }, 100); } async function salvarNovaTarefaDeOS(osId){ const os = osData.find(function(o){ return o.id === osId; }); const desc = document.getElementById('ntos-desc-input').value.trim(); if (!desc) { toast('Escreva o que precisa ser feito', 'err'); return; } const resp = document.getElementById('ntos-resp-input').value.trim(); const prazo = document.getElementById('ntos-prazo-input').value; try { await sbPost('tarefas', { titulo: desc, descricao: 'Criada manualmente na OS #' + ((os && os.numero) || osId.substring(0,6)), cliente_nome: (os && (os.cliente_nome || os.cliente)) || '', responsavel: resp || (ME && ME.nome) || '', prazo: prazo || null, status: 'pendente', prioridade: 'media', origem: 'os_manual', os_gerada_id: osId }); toast('Tarefa criada no Kanban!', 'ok'); const modal = document.getElementById('modal-nova-tarefa-os'); if (modal) modal.remove(); } catch(e) { toast('Erro: ' + e.message, 'err'); } } function renderAgendaMes(){ const labelEl=document.getElementById('agenda-label-semana'); const grade=document.getElementById('agenda-grade'); if(!labelEl||!grade)return; const ano=agendaSemanaBase.getFullYear(); const mes=agendaSemanaBase.getMonth(); const nomesMeses=(LANG==='pt')?['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro']:['January','February','March','April','May','June','July','August','September','October','November','December']; labelEl.textContent=nomesMeses[mes]+' '+ano; const busca=(document.getElementById('agenda-busca') ? document.getElementById('agenda-busca').value : '').toLowerCase(); const prioridadeDotCor={media:'#94a3b8',alta:'#f59e0b',urgente:'#ef4444'}; const diasSemanaLbl=(LANG==='pt')?['Dom','Seg','Ter','Qua','Qui','Sex','Sáb']:['Sun','Mon','Tue','Wed','Thu','Fri','Sat']; const primeiroDoMes=new Date(ano,mes,1); const inicioGrade=new Date(primeiroDoMes); inicioGrade.setDate(inicioGrade.getDate()-primeiroDoMes.getDay()); const hojeISO=agendaFormatarISO(new Date()); let html='<table style="width:100%;border-collapse:collapse;table-layout:fixed;min-width:700px">'; html+='<tr>'+diasSemanaLbl.map(d=>'<td style="text-align:center;font-size:11px;color:#888;padding:6px 4px">'+d+'</td>').join('')+'</tr>'; const cursor=new Date(inicioGrade); for(let semana=0;semana<6;semana++){ html+='<tr>'; for(let dia=0;dia<7;dia++){ const iso=agendaFormatarISO(cursor); const noMes=cursor.getMonth()===mes; const evs=tarefasData.filter(t=>{ if(t.prazo!==iso)return false; if(!t.tecnicos.some(nome=>agendaTecnicosAtivos.includes(nome)))return false; if(!agendaPrioridadesAtivas.includes(t.prioridade||'media'))return false; if(busca && !((t.titulo||'').toLowerCase().includes(busca) || (t.cliente_nome||'').toLowerCase().includes(busca)))return false; return true; }); const corCel=(iso===hojeISO)?'#eef6ff':(noMes?'#fff':'#fafaf9'); const corNum=(iso===hojeISO)?'#2563eb':(noMes?'#888':'#ccc'); html+='<td style="vertical-align:top;padding:5px;height:92px;max-height:92px;overflow:hidden;border:1px solid #f0f0ee;background:'+corCel+'">'+'<div style="font-size:11px;font-weight:'+(iso===hojeISO?'600':'400')+';color:'+corNum+';margin-bottom:3px">'+cursor.getDate()+'</div>'+evs.slice(0,3).map(t=>{ const corTec=agendaCorTecnico(t.tecnicos[0]); return '<div onclick="agendaAbrirTarefa(\''+t.id+'\')" style="cursor:pointer;background:'+corTec+'22;border-left:3px solid '+corTec+';border-radius:4px;padding:2px 5px;margin-bottom:2px;font-size:10px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+t.titulo+'</div>'; }).join('')+(evs.length>3?'<div style="font-size:10px;color:#999">+'+(evs.length-3)+'</div>':'')+'</td>'; cursor.setDate(cursor.getDate()+1); } html+='</tr>'; } html+='</table>'; grade.innerHTML=html; } function renderAgendaHorarios(dias,busca){ const HORA_INICIO=7,HORA_FIM=20,ROW_H=48; const diasISO=dias.map(agendaFormatarISO); const hojeISO=agendaFormatarISO(new Date()); const prioridadeOk=function(t){return agendaPrioridadesAtivas.includes(t.prioridade||'media');}; const tecOk=function(t){return t.tecnicos.some(function(nome){return agendaTecnicosAtivos.includes(nome);});}; const buscaOk=function(t){return !busca || ((t.titulo||'').toLowerCase().includes(busca)||(t.cliente_nome||'').toLowerCase().includes(busca));}; const diasSemanaLbl=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb']; let headerHtml='<div style="display:flex"><div style="width:52px;flex-shrink:0"></div>'+dias.map(function(d){return '<div style="flex:1;text-align:center;padding:6px 4px;border-left:1px solid #f0f0ee"><div style="font-size:11px;color:#888">'+diasSemanaLbl[d.getDay()]+'</div><div style="font-size:14px;font-weight:600;color:#1a1a1a">'+d.getDate()+'</div></div>';}).join('')+'</div>'; let allDayHtml='<div style="display:flex;border-top:1px solid #f0f0ee;border-bottom:1px solid #f0f0ee"><div style="width:52px;flex-shrink:0;font-size:9px;color:#aaa;padding:4px">'+(LANG==='pt'?'Dia todo':'All day')+'</div>'+dias.map(function(d,i){var iso=diasISO[i];var evs=tarefasData.filter(function(t){return t.prazo===iso && !t.hora && tecOk(t) && prioridadeOk(t) && buscaOk(t);});return '<div style="flex:1;border-left:1px solid #f0f0ee;padding:3px;min-height:22px">'+evs.map(function(t){var cor=agendaCorTecnico(t.tecnicos[0]);return '<div onclick="agendaAbrirTarefa(\''+t.id+'\')" style="cursor:pointer;background:'+cor+';color:#fff;border-radius:4px;padding:2px 6px;margin-bottom:2px;font-size:10px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+t.titulo+'</div>';}).join('')+'</div>';}).join('')+'</div>'; const horas=[]; for(let h=HORA_INICIO;h<=HORA_FIM;h++)horas.push(h); let gridHtml='<div style="display:flex;position:relative"><div style="width:52px;flex-shrink:0">'+horas.map(function(h){return '<div style="height:'+ROW_H+'px;font-size:10px;color:#aaa;text-align:right;padding-right:6px;position:relative;top:-6px">'+String(h).padStart(2,'0')+':00</div>';}).join('')+'</div>'; gridHtml+=dias.map(function(d,i){ var iso=diasISO[i]; var evs=tarefasData.filter(function(t){return t.prazo===iso && t.hora && tecOk(t) && prioridadeOk(t) && buscaOk(t);}); var colHtml='<div style="flex:1;position:relative;border-left:1px solid #f0f0ee;'+(iso===hojeISO?'background:#f7fbff;':'')+'height:'+((HORA_FIM-HORA_INICIO+1)*ROW_H)+'px">'; colHtml+=horas.map(function(h,hi){return '<div style="position:absolute;top:'+(hi*ROW_H)+'px;left:0;right:0;height:'+ROW_H+'px;border-top:1px solid #f5f5f3"></div>';}).join(''); colHtml+=evs.map(function(t){ var partesIni=String(t.hora).split(':'); var hIni=parseInt(partesIni[0],10); var mIni=parseInt(partesIni[1]||0,10); var hFim=hIni+1,mFim=mIni; if(t.hora_fim){ var partesFim=String(t.hora_fim).split(':'); hFim=parseInt(partesFim[0],10); mFim=parseInt(partesFim[1]||0,10); } var top=((hIni-HORA_INICIO)+mIni/60)*ROW_H; if(top<0)top=0; var dur=((hFim-hIni)+(mFim-mIni)/60)*ROW_H; if(dur<20)dur=20; var cor=agendaCorTecnico(t.tecnicos[0]); return '<div onclick="agendaAbrirTarefa(\''+t.id+'\')" style="cursor:pointer;position:absolute;top:'+top+'px;height:'+dur+'px;left:2px;right:2px;background:'+cor+'22;border-left:3px solid '+cor+';border-radius:4px;padding:2px 5px;overflow:hidden;font-size:10px;font-weight:500;z-index:2">'+String(t.hora).slice(0,5)+' '+t.titulo+'</div>'; }).join(''); colHtml+='</div>'; return colHtml; }).join(''); gridHtml+='</div>'; document.getElementById('agenda-grade').innerHTML='<div style="overflow-x:auto">'+headerHtml+allDayHtml+'<div style="max-height:520px;overflow-y:auto">'+gridHtml+'</div></div>'; } async function renderDRE(){ const el=document.getElementById('mod-content'); el.innerHTML='<div style="text-align:center;padding:40px;color:#bbb">'+tr('loading')+'</div>'; try { const rows=await sbGet('qb_snapshot?tipo=eq.dre&select=dados,atualizado_em'); if(!rows.length){ el.innerHTML='<div style="text-align:center;padding:60px;color:#bbb"><div style="font-size:36px">📊</div><div style="font-size:14px;font-weight:500;color:#555;margin-top:10px">'+(LANG==='pt'?'Ainda sem dados do QuickBooks':'No QuickBooks data yet')+'</div><div style="font-size:12px;color:#999;margin-top:6px;max-width:360px;margin-left:auto;margin-right:auto">'+(LANG==='pt'?'A sincronizacao automatica ainda nao rodou. Assim que rodar, o DRE aparece aqui.':'The automatic sync has not run yet. Once it does, the P&L will show up here.')+'</div></div>'; return; } const snap=rows[0]; const d=snap.dados; const atualizado=new Date(snap.atualizado_em); const fmt=function(v){ return '$'+Math.round(v).toLocaleString('en-US'); }; const corDica={ alerta:['#fff8e1','#f39c12','#9a6b00','#5c4b1e','⚠'], risco:['#fdeaea','#e74c3c','#a02c2c','#6b2626','💰'], info:['#eaf2fd','#3b82f6','#1d4ed8','#1e3a6d','💳'], neutro:['#f0f0ee','#999','#555','#555','📋'] }; let html=''; html+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><div style="font-size:11px;color:#999">'+(d.periodo||'')+'</div><div style="display:flex;gap:8px;align-items:center"><span style="font-size:11px;background:#e8f5e9;color:#2e7d32;padding:5px 10px;border-radius:99px;font-weight:600;display:inline-flex;align-items:center;gap:5px"><span style="width:6px;height:6px;border-radius:50%;background:#2e7d32;display:inline-block"></span>QuickBooks</span><span style="font-size:11px;color:#999">'+(LANG==='pt'?'Atualizado em ':'Updated ')+atualizado.toLocaleString(LANG==='pt'?'pt-BR':'en-US')+'</span></div></div>'; function kpi(label,valor,sub,corSub,fundo,corTexto){ return '<div style="background:'+fundo+';border-radius:10px;padding:14px;box-shadow:0 1px 3px rgba(0,0,0,.06);color:'+corTexto+'"><div style="font-size:10.5px;color:'+(fundo==='#1a1a1a'?'#bbb':'#888')+';margin-bottom:6px">'+label+'</div><div style="font-size:19px;font-weight:700">'+valor+'</div><div style="font-size:10.5px;margin-top:4px;font-weight:600;color:'+corSub+'">'+sub+'</div></div>'; } html+='<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:16px">' + kpi((LANG==='pt'?'RECEITA TOTAL':'TOTAL REVENUE'), fmt(d.receita), '', '#999', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'CUSTO (CPV)':'COST OF GOODS'), fmt(d.cpv), (d.receita? (d.cpv/d.receita*100).toFixed(1)+'% '+(LANG==='pt'?'da receita':'of revenue'):''), '#999', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'LUCRO BRUTO':'GROSS PROFIT'), fmt(d.lucroBruto), (LANG==='pt'?'margem ':'margin ')+d.margemBruta.toFixed(1)+'%', '#2e7d32', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'DESPESAS OPERACIONAIS':'OPERATING EXPENSES'), fmt(d.despesasOperacionais), (d.receita? (d.despesasOperacionais/d.receita*100).toFixed(1)+'% '+(LANG==='pt'?'da receita':'of revenue'):''), '#999', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'LUCRO LIQUIDO':'NET PROFIT'), fmt(d.lucroLiquido), (LANG==='pt'?'margem ':'margin ')+d.margemLiquida.toFixed(1)+'%', '#8bc98f', '#1a1a1a', '#fff') + '</div>'; html+='<div style="display:grid;grid-template-columns:1.3fr 1fr;gap:12px">'; html+='<div style="display:flex;flex-direction:column;gap:12px">'; html+='<div style="background:#fff;border-radius:10px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.06)"><div style="font-size:12.5px;font-weight:600;margin-bottom:10px">'+(LANG==='pt'?'Receita, despesas e lucro por mes':'Revenue, expenses and profit by month')+'</div><div style="height:200px"><canvas id="dreChartMensal"></canvas></div></div>'; html+='<div style="background:#fff;border-radius:10px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.06)"><div style="font-size:12.5px;font-weight:600;margin-bottom:10px">'+(LANG==='pt'?'Maiores despesas':'Biggest expenses')+'</div><div id="dreListaDespesas"></div></div>'; html+='</div>'; html+='<div style="display:flex;flex-direction:column;gap:10px">' + (d.dicas||[]).map(function(dc){ var c=corDica[dc.cor]||corDica.neutro; return '<div style="background:'+c[0]+';border-left:3px solid '+c[1]+';border-radius:8px;padding:12px"><div style="font-size:11.5px;font-weight:700;color:'+c[2]+';margin-bottom:4px">'+c[4]+' '+dc.titulo+'</div><div style="font-size:11.5px;color:'+c[3]+';line-height:1.5">'+dc.texto+'</div></div>'; }).join('') + '</div>'; html+='</div>'; el.innerHTML=html; if(window.__dreChart){ window.__dreChart.destroy(); } var ctx=document.getElementById('dreChartMensal'); var meses=(d.mensal||[]).map(function(m){return m.mes;}); var receita=(d.mensal||[]).map(function(m){return m.receita;}); var despesas=(d.mensal||[]).map(function(m){return m.despesas;}); var lucro=(d.mensal||[]).map(function(m){return m.lucro;}); window.__dreChart=new Chart(ctx,{ type:'bar', data:{ labels:meses, datasets:[ {type:'bar',label:(LANG==='pt'?'Receita':'Revenue'),data:receita,backgroundColor:'#3b82f620',borderColor:'#3b82f6',borderWidth:1,borderRadius:4,order:2}, {type:'bar',label:(LANG==='pt'?'Despesas':'Expenses'),data:despesas,backgroundColor:'#e74c3c20',borderColor:'#e74c3c',borderWidth:1,borderRadius:4,order:2}, {type:'line',label:(LANG==='pt'?'Lucro liquido':'Net profit'),data:lucro,borderColor:'#1a1a1a',backgroundColor:'#1a1a1a',borderWidth:2,tension:.3,pointRadius:3,order:1} ] }, options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ position:'bottom', labels:{ boxWidth:10, font:{size:10.5} } } }, scales:{ x:{ grid:{ display:false }, ticks:{ font:{size:10.5} } }, y:{ ticks:{ font:{size:10}, callback:function(v){ return '$'+(v/1000)+'k'; } }, grid:{ color:'#f0f0ee' } } } } }); var despesasCat=(d.despesasPorCategoria||[]).slice(0,8); var maxCat=despesasCat.length?despesasCat[0].valor:1; document.getElementById('dreListaDespesas').innerHTML=despesasCat.map(function(dc){ var pct=(dc.valor/maxCat*100).toFixed(0); return '<div style="margin-bottom:9px"><div style="display:flex;justify-content:space-between;font-size:11.5px;margin-bottom:3px"><span>'+dc.nome+'</span><span style="font-weight:600">'+fmt(dc.valor)+'</span></div><div style="background:#f0f0ee;border-radius:4px;height:6px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:#1a1a1a"></div></div></div>'; }).join(''); } catch(e) { el.innerHTML='<div style="text-align:center;padding:40px;color:#e74c3c">'+e.message+'</div>'; } } function renderDespesasGerais(){   despGeralState = { step: 1, base64: null, mediaType: null, file: null };   const el = document.getElementById('mod-content');   el.innerHTML = '<div style="display:flex;gap:18px;margin-bottom:20px">' +       '<div class="desp-step-dot on" data-step="1" style="width:26px;height:26px;border-radius:50%;background:#1a1a1a;color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600">1</div>' +       '<div class="desp-step-dot" data-step="2" style="width:26px;height:26px;border-radius:50%;background:#e8e8e5;color:#888;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600">2</div>' +       '<div class="desp-step-dot" data-step="3" style="width:26px;height:26px;border-radius:50%;background:#e8e8e5;color:#888;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600">3</div>' +     '</div>' +     '<div class="desp-step" id="desp-step-1" style="display:block">' +       '<div style="background:#fff;border:1px solid #e8e8e5;border-radius:10px;padding:20px;margin-bottom:16px">' +         '<div style="font-size:14px;font-weight:600;margin-bottom:12px">Documento fiscal</div>' +         '<label style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;border:2px dashed #e8e8e5;border-radius:10px;padding:30px;cursor:pointer">' +           '<span style="font-size:28px">📷</span><span style="font-size:12px;color:#888">Tirar foto ou escolher arquivo</span>' +           '<input type="file" accept="image/*,application/pdf" style="display:none" onchange="despGeralArquivoLido(this)">' +         '</label>' +         '<img id="desp-preview" style="display:none;max-width:100%;margin-top:14px;border-radius:8px">' + '<div id="desp-preview-doc" style="display:none;align-items:center;gap:8px;margin-top:14px;padding:10px 12px;background:#f5f5f3;border-radius:8px;font-size:12px"><span style="font-size:20px">📄</span><span id="desp-preview-filename"></span></div>' +         '<button class="btn-sec" id="desp-btn-auto" disabled onclick="despGeralPreencherAuto()" style="margin-top:14px;width:100%">Preencher automaticamente</button>' +       '</div>' +       '<button class="btn-pri" id="desp-btn-cont-doc" disabled onclick="despGeralIrPara(2)" style="width:100%">Continuar</button>' +     '</div>' +     '<div class="desp-step" id="desp-step-2" style="display:none">' +       '<div style="background:#fff;border:1px solid #e8e8e5;border-radius:10px;padding:20px;margin-bottom:16px;display:flex;flex-direction:column;gap:12px">' +         '<div><label class="l-lbl">Empresa/Fornecedor</label><input id="desp-empresa" style="width:100%;padding:8px 11px;border:1px solid #e8e8e5;border-radius:7px;font-size:13px;font-family:inherit"></div>' +         '<div><label class="l-lbl">Valor</label><input id="desp-valor" type="number" step="0.01" style="width:100%;padding:8px 11px;border:1px solid #e8e8e5;border-radius:7px;font-size:13px;font-family:inherit"></div>' +         '<div><label class="l-lbl">Data do documento</label><input id="desp-data" type="date" style="width:100%;padding:8px 11px;border:1px solid #e8e8e5;border-radius:7px;font-size:13px;font-family:inherit"></div>' +         '<div><label class="l-lbl">Tipo de documento</label><select id="desp-tipo" style="width:100%;padding:8px 11px;border:1px solid #e8e8e5;border-radius:7px;font-size:13px;font-family:inherit"><option>Recibo</option><option>Fatura</option><option>Nota Fiscal</option><option>Documento</option></select></div>' +         '<div><label class="l-lbl">Itens / Descricao</label><textarea id="desp-descricao" rows="4" placeholder="Ex: 2x Filtro de oleo, Troca de oleo, Mao de obra" style="width:100%;padding:8px 11px;border:1px solid #e8e8e5;border-radius:7px;font-size:13px;font-family:inherit;resize:vertical"></textarea></div>' +       '</div>' +       '<div style="display:flex;gap:10px">' +         '<button class="btn-sec" onclick="despGeralIrPara(1)" style="flex:1">Voltar</button>' +         '<button class="btn-pri" onclick="despGeralIrPara(3)" style="flex:1">Continuar</button>' +       '</div>' +     '</div>' +     '<div class="desp-step" id="desp-step-3" style="display:none">' +       '<div style="background:#fff;border:1px solid #e8e8e5;border-radius:10px;padding:20px;margin-bottom:16px" id="desp-resumo"></div>' +       '<div style="display:flex;gap:10px">' +         '<button class="btn-sec" onclick="despGeralIrPara(2)" style="flex:1">Voltar</button>' +         '<button class="btn-pri" id="desp-btn-salvar" onclick="despGeralSalvar()" style="flex:1">Salvar despesa</button>' +       '</div>' +     '</div>' +     '<div style="margin-top:24px;background:#fff;border:1px solid #e8e8e5;border-radius:10px;overflow:hidden">' +       '<div style="padding:12px 14px;border-bottom:0.5px solid #e8e8e5;font-size:12px;font-weight:500">Despesas Lancadas</div>' +       '<div id="desp-lista"></div>' +     '</div>';   carregarMinhasDespesasGerais(); }
...
const I18N = {
  ...
  agenda_dia: { en: 'Day', pt: 'Dia' },  agenda_mes: { en: 'Month', pt: 'Mês' },
...
function loadModule(id) {
  ...
  else if (id === 'projetos-planta') renderPlantas(); async functelse if (id === 'fin-frota') renderFrota(); else if (id === 'fin-dre') renderDRE();ion renderDRE(){ const el=document.getElementById('mod-content'); el.innerHTML='<div style="text-align:center;padding:40px;color:#bbb">'+tr('loading')+'</div>'; try { const rows=await sbGet('qb_snapshot?tipo=eq.dre&select=dados,atualizado_em'); if(!rows.length){ el.innerHTML='<div style="text-align:center;padding:60px;color:#bbb"><div style="font-size:36px">📊</div><div style="font-size:14px;font-weight:500;color:#555;margin-top:10px">'+(LANG==='pt'?'Ainda sem dados do QuickBooks':'No QuickBooks data yet')+'</div><div style="font-size:12px;color:#999;margin-top:6px;max-width:360px;margin-left:auto;margin-right:auto">'+(LANG==='pt'?'A sincronizacao automatica ainda nao rodou. Assim que rodar, o DRE aparece aqui.':'The automatic sync has not run yet. Once it does, the P&L will show up here.')+'</div></div>'; return; } const snap=rows[0]; const d=snap.dados; const atualizado=new Date(snap.atualizado_em); const fmt=function(v){ return '$'+Math.round(v).toLocaleString('en-US'); }; const corDica={ alerta:['#fff8e1','#f39c12','#9a6b00','#5c4b1e','⚠'], risco:['#fdeaea','#e74c3c','#a02c2c','#6b2626','💰'], info:['#eaf2fd','#3b82f6','#1d4ed8','#1e3a6d','💳'], neutro:['#f0f0ee','#999','#555','#555','📋'] }; let html=''; html+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><div style="font-size:11px;color:#999">'+(d.periodo||'')+'</div><div style="display:flex;gap:8px;align-items:center"><span style="font-size:11px;background:#e8f5e9;color:#2e7d32;padding:5px 10px;border-radius:99px;font-weight:600;display:inline-flex;align-items:center;gap:5px"><span style="width:6px;height:6px;border-radius:50%;background:#2e7d32;display:inline-block"></span>QuickBooks</span><span style="font-size:11px;color:#999">'+(LANG==='pt'?'Atualizado em ':'Updated ')+atualizado.toLocaleString(LANG==='pt'?'pt-BR':'en-US')+'</span></div></div>'; function kpi(label,valor,sub,corSub,fundo,corTexto){ return '<div style="background:'+fundo+';border-radius:10px;padding:14px;box-shadow:0 1px 3px rgba(0,0,0,.06);color:'+corTexto+'"><div style="font-size:10.5px;color:'+(fundo==='#1a1a1a'?'#bbb':'#888')+';margin-bottom:6px">'+label+'</div><div style="font-size:19px;font-weight:700">'+valor+'</div><div style="font-size:10.5px;margin-top:4px;font-weight:600;color:'+corSub+'">'+sub+'</div></div>'; } html+='<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:16px">' + kpi((LANG==='pt'?'RECEITA TOTAL':'TOTAL REVENUE'), fmt(d.receita), '', '#999', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'CUSTO (CPV)':'COST OF GOODS'), fmt(d.cpv), (d.receita? (d.cpv/d.receita*100).toFixed(1)+'% '+(LANG==='pt'?'da receita':'of revenue'):''), '#999', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'LUCRO BRUTO':'GROSS PROFIT'), fmt(d.lucroBruto), (LANG==='pt'?'margem ':'margin ')+d.margemBruta.toFixed(1)+'%', '#2e7d32', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'DESPESAS OPERACIONAIS':'OPERATING EXPENSES'), fmt(d.despesasOperacionais), (d.receita? (d.despesasOperacionais/d.receita*100).toFixed(1)+'% '+(LANG==='pt'?'da receita':'of revenue'):''), '#999', '#fff', '#1a1a1a') + kpi((LANG==='pt'?'LUCRO LIQUIDO':'NET PROFIT'), fmt(d.lucroLiquido), (LANG==='pt'?'margem ':'margin ')+d.margemLiquida.toFixed(1)+'%', '#8bc98f', '#1a1a1a', '#fff') + '</div>'; html+='<div style="display:grid;grid-template-columns:1.3fr 1fr;gap:12px">'; html+='<div style="display:flex;flex-direction:column;gap:12px">'; html+='<div style="background:#fff;border-radius:10px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.06)"><div style="font-size:12.5px;font-weight:600;margin-bottom:10px">'+(LANG==='pt'?'Receita, despesas e lucro por mes':'Revenue, expenses and profit by month')+'</div><div style="height:200px"><canvas id="dreChartMensal"></canvas></div></div>'; html+='<div style="background:#fff;border-radius:10px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.06)"><div style="font-size:12.5px;font-weight:600;margin-bottom:10px">'+(LANG==='pt'?'Maiores despesas':'Biggest expenses')+'</div><div id="dreListaDespesas"></div></div>'; html+='</div>'; html+='<div style="display:flex;flex-direction:column;gap:10px">' + (d.dicas||[]).map(function(dc){ var c=corDica[dc.cor]||corDica.neutro; return '<div style="background:'+c[0]+';border-left:3px solid '+c[1]+';border-radius:8px;padding:12px"><div style="font-size:11.5px;font-weight:700;color:'+c[2]+';margin-bottom:4px">'+c[4]+' '+dc.titulo+'</div><div style="font-size:11.5px;color:'+c[3]+';line-height:1.5">'+dc.texto+'</div></div>'; }).join('') + '</div>'; html+='</div>'; el.innerHTML=html; if(window.__dreChart){ window.__dreChart.destroy(); } var ctx=document.getElementById('dreChartMensal'); var meses=(d.mensal||[]).map(function(m){return m.mes;}); var receita=(d.mensal||[]).map(function(m){return m.receita;}); var despesas=(d.mensal||[]).map(function(m){return m.despesas;}); var lucro=(d.mensal||[]).map(function(m){return m.lucro;}); window.__dreChart=new Chart(ctx,{ type:'bar', data:{ labels:meses, datasets:[ {type:'bar',label:(LANG==='pt'?'Receita':'Revenue'),data:receita,backgroundColor:'#3b82f620',borderColor:'#3b82f6',borderWidth:1,borderRadius:4,order:2}, {type:'bar',label:(LANG==='pt'?'Despesas':'Expenses'),data:despesas,backgroundColor:'#e74c3c20',borderColor:'#e74c3c',borderWidth:1,borderRadius:4,order:2}, {type:'line',label:(LANG==='pt'?'Lucro liquido':'Net profit'),data:lucro,borderColor:'#1a1a1a',backgroundColor:'#1a1a1a',borderWidth:2,tension:.3,pointRadius:3,order:1} ] }, options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ position:'bottom', labels:{ boxWidth:10, font:{size:10.5} } } }, scales:{ x:{ grid:{ display:false }, ticks:{ font:{size:10.5} } }, y:{ ticks:{ font:{size:10}, callback:function(v){ return '$'+(v/1000)+'k'; } }, grid:{ color:'#f0f0ee' } } } } }); var despesasCat=(d.despesasPorCategoria||[]).slice(0,8); var maxCat=despesasCat.length?despesasCat[0].valor:1; document.getElementById('dreListaDespesas').innerHTML=despesasCat.map(function(dc){ var pct=(dc.valor/maxCat*100).toFixed(0); return '<div style="margin-bottom:9px"><div style="display:flex;justify-content:space-between;font-size:11.5px;margin-bottom:3px"><span>'+dc.nome+'</span><span style="font-weight:600">'+fmt(dc.valor)+'</span></div><div style="background:#f0f0ee;border-radius:4px;height:6px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:#1a1a1a"></div></div></div>'; }).join(''); } catch(e) { el.innerHTML='<div style="text-align:center;padding:40px;color:#e74c3c">'+e.message+'</div>'; } } function renderDespesasGerais(){
    ...
    <div style="margin-top:16px;padding-top:14px;border-top:1px solid #e8e8e5">
      ${os.status === 'concluida'
        ...
        : '<div style="background:#eff6ff;border-radius:8px;padding:12px 14px">'
          ...
          + '</div>'}
      <div style="margin-top:10px"><button onclick="abrirNovaTarefaDeOS('${id}')" style="width:100%;padding:9px;border:1px solid #e8e8e5;border-radius:7px;background:#fff;color:#1a1a1a;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit">+ Nova tarefa</button></div><div id="resumo-ia-status-${id}" style="display:none;font-size:11px;color:#2563eb;margin-top:8px"></div>
...
async function deletarNota(notaId, osId){   if (!confirm('Excluir esta anotacao?')) return;   try {     await sbDelete('os_notas?id=eq.' + notaId);     const notas = await sbGet('os_notas?os_id=eq.' + osId + '&order=created_at.asc');     const el = document.getElementById('notas-' + osId);     if (el) el.innerHTML = notas.length ? notas.map(function(n){       return '<div style="background:#f9f9f7;border-radius:8px;padding:10px 12px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:flex-start;gap:8px"><div style="flex:1"><div style="font-size:13px;margin-bottom:3px">'+n.texto+'</div><div style="font-size:10px;color:#bbb">'+(n.autor||'—')+' · '+new Date(n.criado_em||n.created_at).toLocaleString('pt-BR')+'</div></div>'+(ME.funcao==='Gestor'?'<button onclick="deletarNota(\'' + n.id + '\',\'' + osId + '\')" style="background:none;border:none;cursor:pointer;color:#e74c3c;font-size:14px;padding:2px 4px" title="Excluir">🗑️</button>':'')+'</div>';     }).join('') : '<div style="color:#bbb;font-size:12px">Nenhuma anotação.</div>';     toast('Anotacao excluida', 'ok');   } catch(e) { toast('Erro: ' + e.message, 'err'); } } async function salvarNota(osId) {
...
function agendaMudarSemana(delta) {
  if (agendaModoView === 'mes') { agendaSemanaBase.setMonth(agendaSemanaBase.getMonth() + delta); renderAgendaGrade(); return; } agendaSemanaBase.setDate(agendaSemanaBase.getDate() + delta * (agendaModoView === 'dia' ? 1 : 7));
  renderAgendaGrade();
}
...
function renderAgendaGrade() {
  ...
  document.getElementById('agenda-filtro-view').innerHTML =
    ...
    + chipTecnicoHTML('dia', tr('agenda_dia'), agendaModoView === 'dia', 'agendaSetModoView') + chipTecnicoHTML('mes', tr('agenda_mes'), agendaModoView === 'mes', 'agendaSetModoView'); if (agendaModoView === 'mes') { renderAgendaMes(); return; }
  ...
  const busca = (document.getElementById('agenda-busca')?.value || '').toLowerCase(); renderAgendaHorarios(dias, busca); return;
